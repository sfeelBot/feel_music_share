...
## 2026-07-27 (YouTube Data API v3 실연동 — 검색 목업 → 실제 호출)
- 작업: 사용자가 Google Cloud Console에서 발급한 YouTube Data API v3 키(`AIzaSyDf6Y7iMR0qMXnERoBRBueNB46jr_KZY3U`, API 제한만 걸림·애플리케이션 제한 없음)를 `ENV`에 반영하고, `AddTrackModal.tsx`(YouTube 세션 곡 검색)와 `trackMatcher.ts`(혼합 세션 매칭용 YouTube 검색)가 쓰던 정적 목업(`youtubeMockSearch.ts`)을 실제 `search.list` + `videos.list` 2단계 호출로 교체.
  1. `env.ts`에 `YOUTUBE_API_KEY` 필드 추가 — Spotify Client ID/Firebase 클라이언트 설정과 같은 성격("클라이언트 임베드 전제, API 스코프 제한이 실질 방어선")임을 JSDoc에 명시.
  2. 신규 파일 `apps/mobile/src/services/youtube/youtubeSearch.ts` 작성. 기존 `youtubeMockSearch.ts`를 대체하는 이름으로 `youtubeSearch.ts`를 택함(파일명에서 "목업"이라는 임시성 표현을 제거하고, `spotifyWebApi.ts`와 대구를 이루는 `youtubeSearch.ts` 네이밍이 더 자연스럽다고 판단 — 새 파일로 만들고 옛 파일을 삭제하는 편이 git diff에서 "이 파일이 실연동으로 완전히 교체됐다"는 의도를 더 명확히 드러낸다고 봄, in-place 수정보다 선호).
     - `search.list`(`part=snippet&type=video&maxResults=10&q=...&key=...`) 호출 후 videoId 목록을 모아 `videos.list`(`part=contentDetails&id=<comma-separated>&key=...`)로 duration을 보강. `search.list`는 duration을 반환하지 않는다는 공식 제약 그대로 반영.
     - `maxResults=10` 근거: Spotify `SPOTIFY_SEARCH_LIMIT`(10)과 맞춤(같은 `AddTrackModal.tsx` 리스트 UI를 공유하므로 서비스별 결과 개수가 들쭉날쭉하면 UX 일관성이 깨진다는 판단). 쿼터 관점에서도 `search.list` 1회 비용(100유닛)은 `maxResults` 값과 무관하게 고정이라 5~10 사이에서는 쿼터 절감 효과가 없다는 점도 근거에 추가.
     - ISO 8601 duration(`PT4M13S` 등) → ms 변환 유틸 `parseIso8601DurationMs`를 정규식(`/^PT(?:(\d+)H)?(?:(\d+)M)?(?:(\d+)S)?$/`)으로 신규 작성. 형식이 안 맞으면 0으로 폴백(치명적이지 않음 — `trackMatcher.ts`의 duration 점수가 이미 완만한 단계 함수라 오차를 흡수).
     - `serviceTrackId`는 기존 목업과 동일하게 `youtube:video:<videoId>` 형식 유지 — `youtubePlayerStub.ts`의 `extractYoutubeVideoId`가 이 접두사를 그대로 파싱하므로 형식을 바꾸지 않아야 다른 코드를 건드리지 않는다.
     - 에러 처리: `spotifyWebApi.ts`의 `spotifyFetch`/`SpotifyApiError` 패턴을 그대로 미러링(`youtubeFetch`/`YoutubeApiError`) — `!response.ok`면 throw, 네트워크 실패는 fetch 자체가 reject. 쿼터 초과(403 `quotaExceeded`)를 별도로 구분하지 않고 다른 4xx/5xx와 동일하게 취급(이번 라운드는 단순하게 처리하라는 지시에 따름). `AddTrackModal.tsx`가 이미 범용 `try/catch`로 `errorMessage`를 세팅하는 구조라 이 패턴이 그대로 맞아떨어진다.
  3. **디바운스 검토 결과: 추가하지 않음**. `AddTrackModal.tsx`의 `handleSearch`는 Spotify/YouTube 둘 다 검색 버튼 클릭 또는 키보드 "검색" 제출(`onSubmitEditing`)에서만 호출되는 구조였다(타이핑마다 자동 검색하는 구조가 아님) — 이미 사용자 액션 1회당 API 호출 1회로 제한돼 있어 쿼터 걱정과 무관하게 디바운스가 필요 없다고 판단. Spotify 쪽도 동일한 트리거 방식이라 서비스별로 다르게 갈 이유도 없었다.
  4. `AddTrackModal.tsx`의 import를 `searchYoutubeTracksMock` → `searchYoutubeTracks`로 교체.
  5. **범위 확장(지시에 없었지만 필요했던 부분)**: 지시문은 `AddTrackModal.tsx` 교체만 명시했으나, grep 결과 `trackMatcher.ts`(혼합 세션 매칭 로직, `findMatchesOnPlatform`)도 프로덕션 코드에서 `searchYoutubeTracksMock`을 실제로 import·호출하고 있었다(테스트 코드가 아님) — 목업 파일을 삭제하면 이 파일이 깨지므로, 같은 실제 함수(`searchYoutubeTracks`)로 함께 교체했다. `mixedMatching.ts`가 이 경로를 통해 혼합 세션의 YouTube 참여자 매칭에 쓰므로, 매칭 후보도 이제 실제 YouTube 검색 결과 기반으로 계산된다(이전에는 5곡 고정 목업 카탈로그 내에서만 매칭 가능했던 제약이 풀림).
  6. 목업 파일(`youtubeMockSearch.ts`) 삭제 전 `grep -r "youtubeMockSearch\|searchYoutubeTracksMock"`로 전체 재확인 — 코드상 참조는 `AddTrackModal.tsx`/`trackMatcher.ts` 둘뿐이었고 둘 다 교체 완료. 테스트 파일(`__tests__/`) 중 이 목업을 직접 import하는 곳은 없었다(`trackMatcher.test.ts`는 `scoreCandidate`/`rankCandidates`/`confidenceLevelForScore`만 테스트하고 `findMatchesOnPlatform`은 호출하지 않음 — 네트워크 의존 함수라 애초에 유닛 테스트 대상이 아니었음). 문서 내 언급(`docs/qa/spotify-mvp-round1-checklist.md`, `docs/spikes/matching-confidence-benchmark.md`)은 과거 시점 기록이라 그대로 두고, 코드 주석에서 목업 파일을 가리키던 부분(`mixedMatching.ts`, `mockSessionSeed.ts`, `youtubePlayerStub.ts`)만 실연동 상태를 반영하도록 갱신.
- 상태: 완료(검증 대기)
- 변경 파일:
  - 신규: `apps/mobile/src/services/youtube/youtubeSearch.ts`.
  - 삭제: `apps/mobile/src/services/youtube/youtubeMockSearch.ts`.
  - 수정: `apps/mobile/src/config/env.ts`(`YOUTUBE_API_KEY` 추가), `apps/mobile/src/components/AddTrackModal.tsx`, `apps/mobile/src/services/matching/trackMatcher.ts`, `apps/mobile/src/state/mixedMatching.ts`(주석만), `apps/mobile/src/services/session/mockSessionSeed.ts`(주석만), `apps/mobile/src/services/youtube/youtubePlayerStub.ts`(주석만), `docs/decisions-needed.md`(항목 3 삭제 + 번호 재정렬, 항목 4 "실기기 검증"에 "검색은 실연동 완료, 이 항목은 IFrame Player 재생 검증만 남음" 갱신 문구 추가).
- 비고(검증 시 주의, 중요):
  1. **실제 네트워크 호출 검증**: `curl`로 `search.list`(쿼리 "IU 봄날")와 `videos.list`(위에서 얻은 videoId 3개)를 각 1회씩 직접 호출해 응답 형식을 확인함 — 둘 다 예상대로 동작(`search.list`가 `items[].id.videoId`/`snippet.title`/`snippet.channelTitle`/`snippet.thumbnails` 반환, `videos.list`가 `items[].contentDetails.duration`을 `PT1M`/`PT3M25S`/`PT2M19S` 같은 ISO 8601 형식으로 반환 — `parseIso8601DurationMs`가 이 형식들을 올바르게 파싱함을 수동 계산으로 재확인). 이 curl 테스트 자체가 쿼터를 소모했다(`search.list` 100유닛 1회 + `videos.list`는 비교적 저렴, 하루 한도 10,000유닛 대비 미미).
  2. `npx tsc --noEmit`(0 errors), `npx eslint .`(0 errors, 25 warnings — 전부 기존부터 있던 `react-native/no-inline-styles` 관용적 경고, 이번 변경으로 신규 경고 없음), `npx jest`(9 suites / 48 tests 전부 통과, 신규 실패 없음) 확인.
  3. Android 빌드(`assembleDebug`)는 이번 라운드에서 생략 — 순수 JS/TS 변경(신규 네이티브 의존성·설정 파일 변경 없음)이라 네이티브 빌드 산출물에 영향을 줄 수 없다고 판단. 다만 **실기기(에뮬레이터 포함) 상에서 실제로 검색이 되는지는 이 환경에서 검증 불가** — curl로 API 자체는 확인했지만 RN `fetch`가 실제 앱 런타임(네트워크 권한, CORS 유사 제약 없음 확인됨 — YouTube API는 CORS 허용 도메인 제한이 없는 공개 REST API) 안에서 문제없이 동작하는지는 검증 에이전트가 실기기/에뮬레이터로 확인 필요.
...
## 2026-07-28 (Round 19 — `debuggableVariants=[]`로 인한 `__DEV__` 영구 false 버그 수정)
- 작업: Round 18 검증에서 발견된 버그(`docs/qa/spotify-mvp-round1-checklist.md` R18.2~R18.4) 수정 — `apps/mobile/android/app/build.gradle`의 `debuggableVariants = []` 설정이 `assembleDebug` 산출물의 `__DEV__`를 항상 `false`로 만들어, `SpotifyConnectScreen.tsx`의 `__DEV__` 전용 데모 로그인 바이패스("데모로 둘러보기") 섹션이 모든 debug APK에서 렌더링되지 않던 문제.
- 상태: 완료(검증 대기)
- 변경 파일: `apps/mobile/android/app/build.gradle`만 수정 (JS/TS 코드는 건드리지 않음).
- 근본 원인 재확인 및 태스크 이름 검증(추측 아님, 실제로 확인):
  1. `node_modules/@react-native/gradle-plugin/react-native-gradle-plugin/src/main/kotlin/com/facebook/react/TaskConfiguration.kt`(49~80행)를 직접 읽어, `isDebuggableVariant=false`인 경우(`debuggableVariants=[]`라 "debug"도 여기 해당) `createBundle${targetName}JsAndAssets`(`targetName = variant.name.capitalizeCompat()`) 태스크를 등록하면서 `it.devEnabled.set(false)`를 하드코딩함을 확인.
  2. `BundleHermesCTask.kt`(66행)에서 `devEnabled`가 실제로 `abstract val devEnabled: Property<Boolean>` (`@get:Input`)임을 확인, `getBundleCommand()`(150행)에서 이 값이 `--dev <값>` 형태로 Metro CLI에 그대로 전달됨을 확인.
  3. `variant.name.capitalizeCompat()`이 "debug"→"Debug"로 변환하므로 태스크 이름은 `createBundleDebugJsAndAssets`일 것으로 예상 — 추측에 그치지 않고, `JAVA_HOME=D:\Android Studio\jbr`/`ANDROID_HOME`,`ANDROID_SDK_ROOT=E:\Android\Sdk`/`GRADLE_USER_HOME=E:\gradle-home` 환경에서 `cd apps/mobile/android && ./gradlew.bat :app:tasks --all`을 실제로 실행해 `createBundleDebugJsAndAssets`/`createBundleReleaseJsAndAssets` 두 태스크가 정확히 이 이름으로 존재함을 직접 확인함.
- 수정 방식: `debuggableVariants=[]` 자체는 되돌리지 않음(되돌리면 "Unable to load script" 원래 문제 재발 — 기존 40번째 줄 주석에 명시된 트레이드오프). 대신 `android { ... }` 블록 이후, `react { ... }` 블록 이후에 `afterEvaluate { tasks.named("createBundleDebugJsAndAssets", com.facebook.react.tasks.BundleHermesCTask) { devEnabled.set(true) } }`를 추가해 태스크 등록 이후 시점에 `devEnabled`만 재정의. Gradle의 `Property<Boolean>`은 태스크 실행 전까지 재설정 가능하고, AGP의 `AndroidComponentsExtension.onVariants` 콜백(플러그인이 `createBundleDebugJsAndAssets`를 등록하는 지점)은 계약상 빌드 스크립트 자신의 `afterEvaluate`보다 먼저 완료가 보장되므로 타이밍 문제 없음. `createBundleReleaseJsAndAssets`는 전혀 건드리지 않음.
- 검증(지시받은 4가지 전부 직접 실행, 지시대로 커밋은 하지 않음):
  1. 정적 검증(`tsc`/`eslint`/`jest`)은 생략 — 이번 변경이 Gradle 설정 파일 하나(`.gradle`)뿐이라 JS/TS 도구 대상 자체가 없음(`git diff --stat`로 변경 파일이 `app/build.gradle` 하나뿐임을 확인).
  2. **Android 빌드**: `./gradlew.bat clean`(BUILD SUCCESSFUL in 5s) 후 `assembleDebug --no-daemon`으로 완전 재빌드 → **BUILD SUCCESSFUL in 2m 32s**(335 actionable tasks: 285 executed, 50 up-to-date). 이어서 클린 없이 증분 재빌드(`assembleDebug --no-daemon`)도 실행 → **BUILD SUCCESSFUL in 12s**(335 actionable tasks: 32 executed, 303 up-to-date) — 두 경우 모두 정상.
  3. **핵심 검증 — `__DEV__`가 실제로 `true`가 됐는지, 두 가지 독립된 방법으로 확인**:
     - **(a) 빌드 산출물 직접 확인**: `android/app/build/intermediates/sourcemaps/react/debug/index.android.bundle.packager.map`(Metro가 Hermes 변환 전 생성하는, 사람이 읽을 수 있는 JSON 소스맵)을 Node 스크립트로 파싱해 `sources[0]`(`"__prelude__"`, Metro가 매 번들 맨 앞에 삽입하는 런타임 프리앰블 모듈)의 `sourcesContent`를 확인한 결과: `var __BUNDLE_START_TIME__=...,__DEV__=true,process=...` — **debug 번들의 `__DEV__` 리터럴이 실제로 `true`로 박혀있음을 직접 확인**. 같은 방법으로 `android/app/build/intermediates/sourcemaps/react/release/index.android.bundle.packager.map`(별도로 `./gradlew.bat createBundleReleaseJsAndAssets --no-daemon` 실행해 생성, BUILD SUCCESSFUL in 26s)도 확인 → `__DEV__=false` — **release는 전혀 영향받지 않았음을 대조 확인**.
     - **(b) Docker+KVM 에뮬레이터 실기기 수준 검증**(Round 18과 동일 절차, `docs/spikes/docker-virtualization-for-mobile-verification.md`): `budtmo/docker-android:emulator_11.0` 컨테이너(`--device /dev/kvm`, 이전 스파이크에서 캐시된 이미지 재사용)를 기동(부팅 완료까지 로그의 `Boot completed` 확인) → 방금 빌드한 `app-debug.apk`(clean 재빌드 산출물)를 `adb install -r`로 설치(`Success`) → `adb shell monkey -p com.mobile -c android.intent.category.LAUNCHER 1`로 앱 실행 → `uiautomator dump` 좌표 기반으로 온보딩 1/2/3컷을 순서대로 탭해 SpotifyConnect 화면까지 도달 → **`uiautomator dump`의 접근성 트리에 `"⚠ 개발자 전용 (릴리즈 빌드에서 제외됨)"`과 `"데모로 둘러보기 (로그인 건너뛰기)"` 텍스트 노드가 실제로 존재함을 확인**(Round 18은 이 두 노드가 트리에 전혀 없었음 — R18.2). 스크린샷으로도 시각 확인(로그인 버튼 아래 빨간 경고 라벨 + 밑줄 링크 정상 렌더링). 여기서 그치지 않고 **"데모로 둘러보기" 버튼을 실제로 탭**해 로그인 벽을 실제로 우회하는지까지 확인 — 탭 후 `uiautomator dump`에서 `"# 코드로 참여하기"`/`"+ 새 세션 만들기"`/`"로그아웃"` 텍스트가 등장(HomeScreen 도달) → **버그가 렌더링 차원을 넘어 기능적으로도 완전히 고쳐졌음을 end-to-end로 확인**. 세션 전체 `adb logcat -d`에서 `FATAL EXCEPTION`/`AndroidRuntime.*com.mobile`/`ANR in` 패턴 0건(크래시 없음). 검증 종료 후 컨테이너는 `docker rm -f`로 정리.
     - 검증 도중 Docker Desktop이 백그라운드에서 한 차례 중단되는 일이 있었으나(이 작업과 무관, 세션 재개 시점과 맞물린 환경 이슈로 추정) Docker Desktop을 재시작하고 컨테이너를 새로 만들어 절차를 처음부터 다시 수행해 최종 결과에는 영향 없음.
  4. **회귀 확인**: `unzip -l app-debug.apk`로 `assets/index.android.bundle`(2,320,748 bytes)이 여전히 APK 안에 물리적으로 패키징돼 있음을 확인 — "Unable to load script"(Metro 없는 사이드로드 환경에서 번들 못 찾는 문제)가 재발하지 않았다는 근거.
- 비고(검증 에이전트가 참고할 점):
  1. 위 (a)+(b) 두 독립된 검증 모두 일관되게 통과했으므로, 이번 수정이 Round 18이 보고한 버그를 실제로 해결했다고 판단한다. 다만 최종 "완료" 판정은 검증 에이전트 몫이며, 특히 R18에서 미검증으로 남았던 5개 항목(세션 생성/참여/RoomScreen/AddTrackModal/SessionSettingsView — 로그인 벽 때문에 이번에 처음 도달 가능해짐)을 이번 라운드에서 이어서 검증할지는 검증 에이전트/리더 판단.
