...
> 결정 아님(명시): "인증 방식"(Firebase Auth 익명 인증 vs 무인증) 하나는 리더/사용자가 결정해야 하는 사항이며, 이 문서는 두 시나리오 모두를 구체적으로 설계해 비교만 제시한다. 그 외 트리 경로 구조·정렬 키 표현 방식 등은 "구체적으로 제시하라"는 요청에 따라 기획 에이전트의 구체적 제안(권고)으로 제시하되, 구현 단계에서 문제가 발견되면 조정 가능한 설계 수준의 판단임을 명시한다.
