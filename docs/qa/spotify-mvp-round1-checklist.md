...
## Round 15 검증 (Docker+KVM 실기기 급 검증 — 로그인 벽 이전)

> 검증 대상 커밋: `20af688`(HEAD, "Log push to origin/main") — 워킹 트리는 이 커밋과 완전히 동일(`docs/agents/leader-log.md` 미스테이징 변경 1건, 커밋되지 않은 `google-services.json` untracked 파일 제외, 둘 다 앱 코드와 무관).
> 검증일: 2026-07-27
> 검증 담당: 검증(Verification) 서브에이전트
> 지시자: 리더 — 사용자가 "현재까지 기능 Android 검증 시작해"라고 명시적으로 요청, CLAUDE.md에 명문화된 "주요 기능 추가 라운드에서만 Docker+KVM 실기기급 검증" 정책을 이번 라운드에 명시적으로 적용.
> 참고 문서: `docs/spikes/docker-virtualization-for-mobile-verification.md`(이 머신에서 이미 실측된 절차), Round 1~14(지금까지 전부 `assembleDebug` 빌드 성공까지만 확인 — 이번이 최초의 실제 설치·실행·화면 검증).
> 환경: Windows 11 Pro (10.0.26200), Docker Desktop 29.2.0(WSL2 backend), `budtmo/docker-android:emulator_11.0`(로컬 캐시 재사용, 재다운로드 없음) 컨테이너 위 Android 11 에뮬레이터(Samsung Galaxy S10 프로필, 1440x3040). Android 빌드: `JAVA_HOME=D:\Android Studio\jbr`, `ANDROID_HOME`/`ANDROID_SDK_ROOT=E:\Android\Sdk`, `GRADLE_USER_HOME=E:\gradle-home`.
> 검증 방식: 이번 라운드는 정적 코드 리뷰가 아니라 **실제 설치·실행·터치 조작·화면 캡처**로 수행했다(스파이크 문서에 기록된 절차 그대로 재현). 범위는 지시받은 대로 **Spotify 로그인 벽 이전**(스플래시→온보딩 3컷→Spotify 연동 화면+Premium 안내 모달)으로 한정했다 — 실제 Spotify 계정으로 로그인을 시도하지 않았다(보안/계정 소유권 문제, 지시 사항 그대로 준수).

### 0. 빌드 + 설치 준비

| # | 항목 | 결과 | 상세 |
|---|---|---|---|
| R15.0a | 현재 `main` HEAD(`20af688`) 소스로 로컬 직접 빌드 | ✅ 통과 | `cd apps/mobile/android && ./gradlew.bat assembleDebug --no-daemon`(지시된 환경변수) — 최초 실행 `BUILD SUCCESSFUL in 12s`(203 tasks, 23 executed/180 up-to-date, 대부분 Round 14 검증 때 이미 빌드된 캐시 재사용). GitHub Release의 `android-debug-latest` APK는 이번에 전혀 사용하지 않았다(지시대로 로컬 소스 직접 빌드만 사용). |
| R15.0b | APK 무결성 — 컨테이너로 전송된 파일이 손상되지 않았는가 | ✅ 통과(한 차례 이상 현상 발견 후 재확인) | 최초 시도에서 `app-debug.apk`(134,197,646 bytes)를 `docker cp`로 컨테이너에 전송한 직후 로컬 원본 파일의 크기(+612 bytes)와 SHA256이 두 차례의 독립적인 `sha256sum` 호출 사이에 달라지는 현상을 발견 — 원인은 특정하지 못했지만(Docker Desktop/WSL2 마운트 동기화 관련 추정, 이 세션의 셸 상태가 호출마다 초기화되는 환경 특성과 결합됐을 가능성), 안전하게 `rm` 후 `assembleDebug`를 재실행해 APK를 새로 생성하고, 스크래치 디렉터리에 별도 사본을 만들어 `unzip -tq`(zip 무결성)와 `sha256sum` 두 번 연속 실행으로 안정성을 확인한 뒤 그 사본을 컨테이너로 전송했다. 전송 후 컨테이너 내부에서 `sha256sum`으로 재확인한 결과 호스트 사본과 **완전히 일치**(`7b6138b8...`)함을 확인 — 최종적으로 설치된 APK가 손상되지 않았음을 실측으로 보증했다. (참고: 이 현상 자체는 이번 검증의 실패 항목이 아니라 절차상 발견한 환경 이상 징후이며, 재발 시 다음 검증자가 참고할 수 있도록 기록해둔다.) |
| R15.0c | `docker run` 컨테이너 기동 (`--device /dev/kvm`, EMULATOR_DEVICE="Samsung Galaxy S10") | ✅ 통과 | 스파이크 문서와 동일한 커맨드로 기동. Git Bash의 자동 경로 변환이 `/dev/kvm`을 Windows 경로로 오염시켜 최초 시도가 `error gathering device information` 오류로 실패했으나, `MSYS_NO_PATHCONV=1` 환경변수로 우회해 정상 기동 확인(스파이크 문서에는 명시적으로 기록되지 않았던 세부 절차상 함정이라 이번에 새로 기록해둔다). `adb devices` → `emulator-5554 device`(정상). |
| R15.0d | `adb install -r`(컨테이너 내부 adb 서버 경유) | ✅ 통과 | `Performing Streamed Install / Success`. |
| R15.0e | `adb shell pm list packages \| grep com.mobile` | ✅ 통과 | `package:com.mobile` 확인. |

### 1. 런칭 + 스플래시 (2.1)

| # | 항목 | 결과 | 상세 |
|---|---|---|---|
| R15.1 | `monkey -p com.mobile -c android.intent.category.LAUNCHER 1`로 런처 실행 | ✅ 통과 | 크래시 없이 실행됨(아래 R15.9 logcat 확인과 함께 재확인). |
| R15.2 | 스플래시 화면 시각 확인 — 로고("S" 마크)/앱이름("Samewave")/태그라인("장거리에서도, 같은 순간에")이 실제로 렌더링되는가 | ✅ 통과 | 콜드 스타트 2초 시점 스크린샷에서 코랄색 원형 로고 마크("S"), "Samewave" 타이틀(대문자 S, 볼드), 태그라인 "장거리에서도, 같은 순간에"가 모두 정확히 렌더링됨을 시각적으로 확인. 하단에 `ActivityIndicator` 스피너도 프레임 중간(")" 모양 글리프로 캡처)에서 확인 — `SplashScreen.tsx` 코드와 텍스트/레이아웃이 정확히 일치. |
| R15.3 | 최소 노출 시간(900ms) 후 Onboarding으로 자동 전환되는가 — 시간차 스크린샷 2장 | 🟡 통과(단, 이 환경 고유의 관찰 사항 포함) | 시간차 스크린샷 시퀀스로 확인: t≈0.3s/0.8s 시점에는 흰 화면(네이티브 부트 스크린, JS 번들 아직 미실행), **t≈2s 시점에 비로소 Samewave 스플래시가 렌더링**(R15.2), t≈3.5s 시점에는 이미 Onboarding 1컷으로 전환 완료. 즉 "스플래시 렌더링 → 900ms 후 자동 전환"이라는 코드 로직(`SPLASH_MIN_DISPLAY_MS=900`) 자체는 확인됐으나(2s~3.5s 사이 어느 시점에 컴포넌트가 마운트되고 900ms 타이머가 돌아 전환됐다는 것은 시간 간격상 일관됨), 이 Docker+KVM 에뮬레이터 환경에서는 **콜드 스타트 시 JS 번들 초기화 자체가 900ms보다 훨씬 오래(약 2초) 걸려**, "네이티브 흰 화면 → 실제 스플래시 텍스트 렌더링"까지의 체감 로딩이 실기기보다 느리게 보일 수 있다는 환경 특성을 발견했다 — 이는 앱 코드의 결함이 아니라 컨테이너 에뮬레이터의 콜드 스타트 성능 특성으로 판단되며(디버그 빌드+에뮬레이션 오버헤드), 사용자의 실제 기기에서는 이보다 빠를 가능성이 높다는 점을 명시해둔다(추정, 이번 라운드에서 실기기와 비교 실측은 하지 않음). |

### 2. 온보딩 3컷 (2.2)

| # | 항목 | 결과 | 상세 |
|---|---|---|---|
| R15.4 | 1컷: 제목/본문 한글 렌더링, 폰트 깨짐 없음 | ✅ 통과 | "같은 곡을, 같은 순간에" / "멀리 있어도 함께 듣는 방을 만들어보세요." — 한글 완전 렌더링, 자소 분리·깨짐·네모 박스(tofu) 없음. 하단 점 인디케이터 3개 중 첫 번째만 강조색. |
| R15.5 | "다음" 버튼 탭으로 실제 화면 전환(uiautomator dump로 정확한 버튼 좌표 파악 후 `input tap`) | ✅ 통과 | 최초 추정 좌표(화면 비율 기반)로는 탭이 반응하지 않는 현상을 발견했으나(중요: 스크린샷 표시 크기와 실제 기기 해상도 간 좌표 환산 실수였을 가능성 — 실기기 좌표계 자체의 문제는 아님), `uiautomator dump`로 버튼의 정확한 `bounds`를 추출한 뒤 그 중심 좌표로 재시도하자 정상적으로 컷 전환됨을 확인. 이는 이번 검증 절차상의 시행착오이며 앱 자체의 버그가 아니다. |
| R15.6 | 2컷: 제목/본문 한글 렌더링 | ✅ 통과 | "누구나 곡을 추가해요" / "함께 만드는 플레이리스트 — 누가 골랐는지 항상 표시돼요." — 정상 렌더링, 점 인디케이터 두 번째로 이동. |
| R15.7 | 3컷(투명성 카드): 도달 확인, "Spotify로 시작하기" CTA 렌더링 | ✅ 통과 | "재생은 각자의 Spotify 앱에서 이뤄져요. feel_music_share는 그걸 맞춰주는 역할이에요." + 본문 문단 전부 정상 렌더링(영문 코드네임 "feel_music_share"과 한글이 한 문장에 섞여도 줄바꿈/렌더링 깨짐 없음), 두 개의 원형 아이콘 + "⇄" 링크 아이콘 일러스트 정상 표시, 점 인디케이터 세 번째로 이동, 하단 "Spotify로 시작하기" CTA가 브랜드 코랄색 pill 버튼으로 렌더링됨. |

### 3. Spotify 연동 화면 + Premium 안내 모달 (2.3, Round 9 수정 사항)

| # | 항목 | 결과 | 상세 |
|---|---|---|---|
| R15.8 | "Spotify로 시작하기" 탭 → SpotifyConnect 화면 전환, "Spotify로 로그인" 버튼/"Premium이 없으신가요? →" 링크 렌더링 | ✅ 통과 | 초록색 원형 음표 아이콘, "feel_music_share는 Spotify Premium 계정이 필요해요" 타이틀, 불릿 2개, 초록색 pill "Spotify로 로그인" 버튼, 하단 "Premium이 없으신가요? →" 링크 모두 화면에 정상 렌더링됨을 확인. |
| R15.9 | "Premium이 없으신가요?" 링크를 `input tap`으로 실제 탭 → 안내 모달이 뜨는가(Round 9 수정 사항, 실기기 수준 검증) | ✅ 통과 | `uiautomator dump`로 정확한 링크 좌표 확보 후 탭 → 모달이 페이드인으로 실제로 나타남을 스크린샷으로 확인: 제목 "Free 계정이어도 괜찮아요", 본문(Free 계정으로도 로그인·참여·곡 추가/삭제/순서변경 가능하나 동기화 재생은 Premium 전용이라는 안내) 전체가 줄바꿈 포함 깨짐 없이 렌더링, 초록색 pill "로그인 계속하기" 버튼, 아웃라인 pill "Spotify Premium 알아보기" 버튼, 하단 "닫기" 텍스트 링크까지 4개 요소 모두 화면에 존재함을 확인 — Round 9에서 수정된 기능이 실제 기기 렌더링 수준에서 정확히 동작함을 최초로 실측 확인했다(이전까지는 코드 리뷰로만 확인됨). |
| R15.10 | "닫기" 탭 → 모달이 실제로 닫히는가 | ✅ 통과 | `uiautomator dump`로 "닫기" 텍스트 좌표 확보 후 탭 → 모달이 사라지고 SpotifyConnect 화면(로그인 버튼/Premium 링크)이 다시 노출됨을 스크린샷으로 확인. `Modal`의 상태(`freeInfoVisible`)가 실제 터치 이벤트에 반응해 정확히 토글됨을 실기기 수준에서 확인. |

### 4. 다크모드 대응

| # | 항목 | 결과 | 상세 |
|---|---|---|---|
| R15.11 | `adb shell cmd uimode night yes`로 다크모드 강제 전환 | ✅ 통과 | Android 11 에뮬레이터에서 `cmd uimode night yes` 명령이 `Night mode: yes`로 정상 응답(사용자가 제안한 `settings put secure ui_night_mode 2` 대신 더 안정적인 `cmd uimode` 인터페이스를 사용 — 동일한 효과, API 30에서 권장되는 방식이라 대체함). |
| R15.12 | 다크모드에서 온보딩 1컷 재확인 | ✅ 통과 | 배경이 네이비/다크 퍼플 톤으로 전환, 텍스트가 흰색으로 반전, 브랜드 강조색(코랄 점 인디케이터)은 라이트모드와 동일하게 유지되어 명도 대비 확보. "다음" 버튼 테두리도 다크 테마 톤으로 자연스럽게 적용됨. 레이아웃 깨짐이나 대비 문제 없음. |
| R15.13 | 다크모드에서 SpotifyConnect 화면 재확인 | ✅ 통과 | 다크 배경 위에 초록색 Spotify 브랜드 버튼, 흰색 타이틀/불릿 텍스트, 회색조 보조 텍스트("Premium이 없으신가요?") 모두 가독성 있게 렌더링. |
| R15.14 | 다크모드에서 Premium 안내 모달 재확인 | ✅ 통과 | 모달 카드 배경이 어두운 톤(`theme.bgElevated`)으로 전환되고 오버레이(반투명 검정)와 카드 사이 경계가 명확히 구분됨, 본문/버튼 텍스트 대비 충분. 라이트모드와 동일한 4개 요소(제목/본문/버튼 2개/닫기) 전부 정상 렌더링. |

### 5. 뒤로가기 / 화면 전환

| # | 항목 | 결과 | 상세 |
|---|---|---|---|
| R15.15 | Premium 모달이 열린 상태에서 Android 뒤로가기(`input keyevent 4`) | ✅ 통과 | 모달이 크래시 없이 닫히고 SpotifyConnect 화면으로 정확히 복귀 — `Modal`의 `onRequestClose` 핸들러가 Android 하드웨어 뒤로가기 이벤트에도 정상적으로 반응함을 확인(단순 탭으로 닫는 R15.10과 별개의 경로). |
| R15.16 | 연속 뒤로가기 4회(SpotifyConnect → 온보딩 3컷 → 2컷 → 1컷 → 앱 종료) | ✅ 통과 | 매 단계 크래시 없이 정확한 이전 화면으로 순차 전환됨을 확인, 최종적으로 React Navigation 스택이 바닥나면서 앱이 종료되고 홈 런처(배경화면)로 빠져나감 — React Navigation의 표준 back-stack 동작과 일치, 버그 아님. 전 과정에서 `adb shell pidof com.mobile`로 프로세스가 유지되고 있었음도 확인(강제 종료/재시작 없이 정상적인 백그라운드 전환). |

### 6. 크래시 여부 (logcat 전수 확인)

| # | 항목 | 결과 | 상세 |
|---|---|---|---|
| R15.17 | 전체 검증 과정(설치~다크모드~뒤로가기) 동안 `adb logcat` 캡처 후 크래시 마커 검색 | ✅ 통과(크래시 없음) | 세션 시작 시 `adb logcat -c`로 초기화 후 백그라운드로 전체 캡처(3,237줄), `FATAL EXCEPTION`/`AndroidRuntime`/`ANR`/`Crash`(대소문자 무관) 패턴 검색 결과 `com.mobile` 프로세스와 관련된 크래시는 0건 — 매칭된 `AndroidRuntime` 라인은 전부 이번 검증에서 사용한 adb shell 명령(`monkey`/`input`/`uiautomator`) 자체의 정상적인 프로세스 시작/종료 로그였다. `Process: com.mobile`/`E AndroidRuntime`/`Force finishing activity com.mobile`/`ANR in com.mobile` 패턴도 0건. `grep -i "com.mobile" ... " E "` (에러 레벨 로그 필터)도 0건 — com.mobile 관련 에러 로그 자체가 전혀 없었다. |
| R15.18 | (관찰, 실패 아님) `ReactNoCrashSoftException: raiseSoftException(onWindowFocusChange...)` 반복 발생 | 📝 관찰 | 콜드 스타트마다 `E/unknown:ReactHost` 태그로 `com.facebook.react.bridge.ReactNoCrashSoftException: ... Tried to access onWindowFocusChange while context is not ready` 로그가 1회씩 발생. 이름 자체가 "NoCrashSoftException"으로 React Native 프레임워크가 "이건 크래시가 아니다"라고 명시하는 소프트 경고이며, 윈도우 포커스 이벤트가 React 컨텍스트 초기화 완료 이전에 도착할 때 발생하는 RN 진영에 알려진 흔한 콜드 스타트 로그다. 실제 크래시나 기능 이상으로 이어지지 않음(R15.2~R15.17 전부 통과)을 확인했으므로 실패로 처리하지 않되, 다음 라운드에서도 반복 관찰되면 패턴으로 기록해 둘 가치가 있어 남긴다. |

### Round 15 종합

| 구분 | 개수 |
|---|---|
| ✅ 통과 | 17 (R15.0a~R15.17, R15.3의 환경 관찰 포함 광의로는 18) |
| 📝 관찰(실패 아님) | 1 (R15.18) |
| 실패 | 없음 |

**결론: 통과.** Round 1~14 내내 "빌드 성공까지만" 확인되고 실제 설치·실행·화면 렌더링은 한 번도 검증되지 않았던 갭을, 이번 라운드에서 Docker+KVM 에뮬레이터(`budtmo/docker-android:emulator_11.0`, 이 머신에서 스파이크로 이미 실측된 절차)로 처음 채웠다. 로그인 벽 이전 전체 흐름(스플래시 → 온보딩 3컷 → Spotify 연동 화면 → Free 계정 안내 모달 열기/닫기, 탭+뒤로가기 양쪽 경로)이 라이트/다크 모드 양쪽에서 실제 터치 조작과 화면 캡처로 정확히 동작함을 확인했다. 특히 Round 9에서 수정된 "Premium이 없으신가요?" 모달은 이번이 최초의 실기기 수준 검증이었는데, 실제로 탭했을 때 모달이 뜨고 버튼 3개가 정확히 렌더링되며 닫기(탭/뒤로가기 양쪽)도 정상 동작함을 확인했다(R15.9/R15.10/R15.15). 한글 텍스트는 온보딩 3컷·Spotify 연동 화면·모달 전체에서 폰트 깨짐 없이 정상 렌더링됐다. 전체 검증 과정에서 `com.mobile` 프로세스의 크래시는 logcat 전수 검색으로 0건임을 확인했다(R15.17). 절차상 두 가지를 기록해둔다: (1) 콜드 스타트 JS 번들 로딩이 이 컨테이너 환경에서 900ms보다 훨씬 오래(~2초) 걸려 스플래시 자동 전환 타이밍의 "체감"이 실기기와 다를 수 있다는 환경 특성(R15.3, 앱 결함 아님), (2) `docker cp`로 APK를 전송하는 과정에서 한 차례 파일 크기/체크섬 불일치 현상을 발견해 재빌드+무결성 재확인으로 우회했다(R15.0b, 원인 미상이나 최종 설치본은 체크섬 일치로 보증됨).

**중요 — 이번 라운드가 채우지 못한 검증 공백 (숨기지 않고 명시):** Spotify 로그인 벽 이후의 모든 기능(세션 생성, Now Playing 재생 컨트롤, 플레이리스트 추가/삭제/순서변경, 참여자 관리/역할 배지, 서비스 전환(Spotify↔YouTube), 동기화 상태 배지, Free 계정 배너, 매칭 흐름 등 — Round 1~14가 코드 리뷰/단위 테스트/코드 트레이스만으로 확인해온 모든 기능)은 이번 라운드에서도 실제 기기 실행 검증이 되지 않았다. 이는 자동화된 에이전트가 실제 Spotify 계정 자격증명으로 로그인하는 것이 보안/계정 소유권상 부적절하기 때문에 의도적으로 범위에서 제외한 것이며, 다음과 같은 후속 조치가 필요하다:

- **사용자가 실기기(또는 이 Docker+KVM 환경)에서 본인의 Spotify 계정으로 직접 로그인한 뒤** 이어서 확인해야 할 목록:
  1. Spotify OAuth 로그인 자체가 실제로 완료되는가(커스텀 URL 스킴 딥링크 콜백 등록 여부 — Round 1부터 "미등록"으로 계속 기록되어 온 TODO, 이번에도 재확인하지 못함).
  2. 세션 생성 화면(정원 스테퍼, 서비스 라디오, 안내 배너)의 실제 터치 상호작용.
  3. Now Playing 화면 — 재생/일시정지/이전곡/다음곡 버튼, 진행바, 동기화 상태 배지 4단계, Free 계정 배너, 참여자 아바타 스택의 실제 렌더링과 탭 반응.
  4. 플레이리스트 탭 — 곡 검색/추가(Spotify Web API 실호출), 롱프레스 삭제, ▲/▼ 순서변경 버튼의 실제 터치 동작.
  5. 참여자 바텀시트 — 역할 배지, 방장 전용 관리자 임명/해제 메뉴의 실제 동작.
  6. 서비스 전환 확인 다이얼로그 → 전환 중 오버레이 → 완료 토스트 흐름, 서비스별 플레이리스트 격리(Round 13에서 코드 트레이스로만 확인)의 실제 화면 전환.
