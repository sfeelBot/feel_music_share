...
1. **Spotify Developer 앱 — User Management에 테스트 계정 추가 필요** (developer.spotify.com/dashboard) — (2026-07-26 실기기 확인) Client ID·리다이렉트 URI 설정은 전부 정상 동작 확인됨(실기기에서 실제로 Spotify OAuth 화면까지 정상 도달) — **다만 로그인 시도 시 "The user is not registered for this application. Please check your settings on https://developer.spotify.com/dashboard." 오류 발생**. 이 앱이 아직 Development Mode(등록된 사용자만 로그인 가능, 최대 25명)라 로그인 시도한 Spotify 계정이 허용 목록에 없어서 막힌 것 — 코드 문제 아님. **해결**: Dashboard → 해당 앱 → Settings → User Management → "Add New User"로 테스트할 Spotify 계정 이메일/이름 추가. (나중에 정식 배포 시에는 Spotify에 Extended Quota Mode 신청 필요 — 지금 단계는 급하지 않음.)
