...
A. **RTDB 보안 규칙 — 인증 방식 결정 (2026-07-27 신규)** — `docs/specs/10-rtdb-schema-and-security-rules.md`(planner 산출물) 참고. `sessionService.ts`를 실제 RTDB로 교체하기 전에 반드시 정해야 함. 두 시나리오:
   - **시나리오 A(Firebase Auth 익명 인증)**: `04-playlist.md`가 이미 확정한 3단계 권한 체계(방장만 관리자 임명 등)를 실제로 서버 측에서 강제 가능. 대신 `@react-native-firebase/auth` 신규 설치 + `participantId`를 `auth.uid`로 통일하는 설계 변경이 필요(파급 범위 있음).
   - **시나리오 B(무인증, 세션 코드 기반)**: 추가 비용 없이 지금 방식 그대로. 대신 방장 사칭·타인 매칭 상태 조작 등을 서버 측에서 막을 방법이 원천적으로 없음 — 권한 체계가 클라이언트 신뢰 수준에 머무름.
   - 이 앱의 실제 사용 맥락("장거리 연인·친구 소규모 세션")상 시나리오 B도 당장 큰 사고로 이어질 가능성은 낮다는 게 planner의 참고 의견이나, 최종 선택은 사용자 몫.
B. **호스트 마이그레이션 시 새 방장 선출 규칙** (`docs/specs/04-playlist.md` "확인 필요" 1번, 2026-07-25부터 미확정 상태로 남아있던 항목 — RTDB 4라운드(참여자/역할) 착수 전 필요해져 재상기) — 방장이 이탈했을 때 (a) 기존 관리자 중 한 명이 우선 승계되는지 (b) 임의 참여자(예: 참여 순서 최고참)가 승계되는지, 선출 알고리즘 자체가 아직 안 정해짐. ("승계 후 기존 관리자 목록 유지"는 이미 확정됨 — 이 항목은 그와 별개로 "누가" 승계되는지만 다룸.)
