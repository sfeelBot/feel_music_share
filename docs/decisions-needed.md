...
1. **Spotify Developer 앱 등록** (developer.spotify.com/dashboard) — (2026-07-26) Client ID 발급·반영 완료(커밋 `229a2bc`), 사용자가 리다이렉트 URI 2개(`feelmusicshare://spotify-auth-callback`, `feelmusicshare://spotify-remote-callback`)도 Dashboard에 등록했다고 확인. 앱 쪽 커스텀 URL 스킴 등록(Android `build.gradle`의 `appAuthRedirectScheme`, iOS `Info.plist`의 `CFBundleURLSchemes`)도 `feelmusicshare`로 일치함을 리더가 재확인 — **다만 Dashboard 등록 내용 자체는 API로 조회할 방법이 없어 원격으로 검증 불가**. 실제 동작 확인은 실기기 로그인 시도가 필요(아래 5번 실기기 검증과 함께 진행 권장) — 그 전까지는 "설정 완료로 보이나 실기기 확인 전"으로 취급.
