...
A. **로그인/인증 흐름 재설계 (2026-07-28 신규, Spotify 완전 삭제로 인해 발생)** — `docs/specs/11-youtube-only-migration-plan.md` 5절 참고. Spotify OAuth가 지금까지 앱의 유일한 "로그인" 수단이었는데, Spotify 자체가 삭제되면서 이 판단 근거가 통째로 사라진다. **삭제 작업의 "라운드 3"(인증 흐름)은 이 결정 없이는 착수 불가** — 세 가지 선택지:
   - **(a) 닉네임 입력만 받는 간단한 온보딩**: Firebase 익명 인증(이미 동작 중)으로 `uid` 자동 발급 + 최초 1회 닉네임 입력만 받음. 구현 가장 단순, 진입 마찰 최소. 단점: 기기/설치가 바뀌면 완전히 새 신원(연속성 없음).
   - **(b) 익명 인증 + 최초 1회 프로필(닉네임/아바타) 설정 화면**: (a)와 비슷하나 정식 온보딩 단계로 명시, 아바타 커스터마이징 등 확장 여지.
   - **(c) 실제 OAuth 로그인(예: Google Sign-In)으로 대체**: 기기를 바꿔도 신원 유지, "진짜 로그인"이라는 기대에 부합. 단, Google Cloud Console OAuth 클라이언트 등록(Android SHA-1 지문 포함)·신규 네이티브 의존성 추가 등 구현 비용이 셋 중 가장 크고, 장거리 연인/친구가 가볍게 세션 하나 여는 이 앱의 실사용 맥락에 로그인 마찰이 안 맞을 수도 있음.
   - 무엇을 고르든 Premium/Free 개념(`accountTier`)은 함께 제거됨(Spotify Premium에서 유래한 개념이라 YouTube 단일 앱에서 의미 없음).
   - 참고: 삭제 작업의 "라운드 1"(데이터 모델)과 "라운드 2"(화면 UI 단순화)는 이 결정과 무관하게 이미 착수 가능 상태.
