...
8. 백그라운드 서브에이전트가 계정 API 세션 한도로 실패했다는 task-notification을 받으면(또는 리더 자신이 같은 오류를 겪으면) `session-limit-recovery` 스킬을 따른다 — 새 작업 분배를 잠시 멈추고, `git status`로 남은 작업을 확인해 커밋 가능한 부분은 커밋하고, `leader-log.md` "현재 상황 요약"에 정확한 재개 지점을 기록한다. 이건 사전 감지가 아니라 사후 복구다(Claude Code에 "사용량 N% 도달" 훅 이벤트 자체가 없음) — 한계를 사용자에게 숨기지 않는다.
9. `docs/roadmap.md`(화면별 기능 현황 + 진행 순서)는 `decisions-needed.md`와 같은 성격의 **살아있는 문서**다 — 화면/기능의 상태가 바뀔 때마다(구현 완료, 검증 통과, 새 갭 발견 등) 리더가 직접 해당 항목을 갱신한다(체크박스 토글, 상태 아이콘 교체, 완료 항목은 "완료된 것" 절로 이동). append-only 로그가 아니므로 과거 상태를 지우고 최신 상태로 덮어써도 된다 — 상세 이력은 `docs/agents/*-log.md`/`docs/qa/`에 이미 남아있다.
...
- **Android 검증 수준(2026-07-27 확정)**: 기본은 빌드 성공(`assembleDebug`) 확인까지다. 리더가 판단하기에 "주요 기능 추가"에 해당하는 라운드는 verifier에게 Docker+KVM 기반 실제 설치/실행/화면 검증(`docs/spikes/docker-virtualization-for-mobile-verification.md` 절차)까지 명시적으로 요청한다 — 사소한 버그 수정·문서 정정 등은 기존처럼 빌드 확인만으로 충분하다.
